...
- **Clickjacking protection requires a hosting layer that sends HTTP response headers.** The CSP `frame-ancestors` directive is silently ignored by browsers when delivered via `<meta>` tag (spec: `Content-Security-Policy` `frame-ancestors` only takes effect from response headers). GitHub Pages does not allow custom response headers, so the shipped meta CSP omits it honestly rather than claim protection it doesn't deliver. To enforce, deploy behind Netlify (`_headers` file), Cloudflare Workers, or a reverse proxy that can inject `Content-Security-Policy: frame-ancestors 'none'` or `X-Frame-Options: DENY`.
